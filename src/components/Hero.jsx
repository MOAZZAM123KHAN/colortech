...
export default function Hero({ activeSlide, setActiveSlide }) {
  ...
  return (
    <section
      id="hero"
      className="relative overflow-hidden pt-24 text-[#F5F0E8] sm:pt-28"
    >
      {/* Background Video */}
      <video
        autoPlay
      ...
      {/* Dark Overlay */}
      <div className="absolute inset-0 bg-[#07111B]/75 z-[1]" />

      {/* Grid Pattern Overlay */}
      <div className="pointer-events-none absolute inset-0 opacity-20 z-[2]">
        <div
          className="h-full w-full"
          style={{
            backgroundImage:
      ...
      {/* Circle Decoration */}
      <div className="pointer-events-none absolute right-[-140px] top-1/2 hidden h-[580px] w-[580px] -translate-y-1/2 lg:block z-[2]">
        <div className="absolute inset-0 rounded-full border border-[#C9A84C]/30" />
        <div className="absolute inset-[60px] rounded-full border border-[#C9A84C]/20" />
        <div className="absolute inset-[120px] rounded-full border border-[#C9A84C]/15" />
      </div>
      ...
      {/* Main Content */}
      <div className="relative z-[3] mx-auto flex min-h-[calc(100vh-6rem)] w-full max-w-7xl items-center px-4 pb-12 sm:px-6 sm:pb-16 md:pb-24 lg:px-10 lg:pb-32">
        <div className="max-w-3xl">
          <span className="inline-flex rounded-full border border-[#C9A84C] px-3 py-1 text-[10px] uppercase tracking-[0.2em] text-[#C9A84C] sm:px-4 sm:text-xs">
            {slide.industry}
          </span>
          ...
          <p
            className="mt-5 max-w-2xl text-base text-[#F5F0E8]/80 sm:text-lg"
            style={{
              animation: "fadeUp 800ms ease 280ms forwards",
              opacity: 0,
            }}
          >
            {slide.subtitle}
      ...
      {/* Slider Dots */}
      <div className="absolute bottom-[88px] left-4 z-[3] flex items-center gap-2 sm:bottom-24 sm:left-6 md:left-10">
        {heroSlides.map((_, idx) => (
          <button
            key={idx}
            onClick={() => setActiveSlide(idx)}
      ...
      {/* Bottom Stats */}
      <div className="relative z-[3] border-t border-white/10 bg-[#0D1B2A]/75 backdrop-blur-sm md:absolute md:bottom-0 md:left-0 md:right-0">
        <div className="mx-auto grid max-w-7xl grid-cols-1 gap-3 px-4 py-4 text-center sm:grid-cols-2 sm:gap-4 sm:px-6 md:grid-cols-4 md:gap-6 md:py-5 lg:px-10">
          {[
            "HTL • Foil • IML",
            "High-Resolution Printing",
